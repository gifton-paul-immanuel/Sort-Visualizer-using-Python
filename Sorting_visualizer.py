...
#Setting speed above 1 is terribly slow (depends on array_size and your patience)!
def set_speed():
    slow = 0.5
    medium = 0.05
    fast = 0.0000001
...
def sort():
    ...
    if algo_comboBox.get() == 'Merge Sort':
        ...
        for i in range(1, len(arr)):
            ...
            arr[j + 1] = key
        displayArr(arr, ["blue" for x in range(len(arr))])
display_window = Frame(root, width=900, height=300, bg="black")
display_window.grid(row=0, column=0, padx=10, pady=5)
lbl1=Label(display_window, text="Algorithm: ", bg="red")
...
lbl2=Label(display_window, text="Sorting Speed: ", bg="red")
...
lbl3= Label(display_window, text="Gifton Paul Immanuel B", bg="black", fg="white")
lbl3.grid(row=0, column=10)
lbl4= Label(display_window, text="URK21CS5054", bg="black", fg="white")
